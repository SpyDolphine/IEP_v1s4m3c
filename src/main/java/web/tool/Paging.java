...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
   ...
   * @return 페이징 생성 문자열
   */ 
  public String paging4(int totalRecord, int nowPage, int recordPerPage, String col, String word, String ch){ 
    int pagePerBlock = 10; // 블럭당 페이지 수 
    int totalPage = (int)(Math.ceil((double)totalRecord/recordPerPage)); // 전체 페이지  
    int totalGrp = (int)(Math.ceil((double)totalPage/pagePerBlock));// 전체 그룹 
    int nowGrp = (int)(Math.ceil((double)nowPage/pagePerBlock));    // 현재 그룹 
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동 
    if (nowGrp >= 2){ 
      str.append("<span class='span_box_1'><A href='./list.do?tp_ch="+ch+"&col="+col+"&word="+word+"&nowPage="+_nowPage+"'>이전</A></span>"); 
    } 
    ...
    for(int i=startPage; i<=endPage; i++){ 
      ...
      if (nowPage == i){ 
        ...
        str.append("<span class='span_box_1'><A href='./list.do?tp_ch="+ch+"&col="+col+"&word="+word+"&nowPage="+i+"'>"+i+"</A></span>");   
      } 
    } 
    
    _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동 
    if (nowGrp < totalGrp){ 
      str.append("<span class='span_box_1'><A href='./list.do?tp_ch="+ch+"&col="+col+"&word="+word+"&nowPage="+_nowPage+"'>다음</A></span>"); 
    } 
    str.append("</DIV>"); 
   ...
   * @return 페이징 생성 문자열
   */ 
  public String paging725( int totalRecord, int nowPage, int recordPerPage, String col, String word){ 
    int pagePerBlock = 12; // 블럭당 페이지 수 
    int totalPage = (int)(Math.ceil((double)totalRecord/recordPerPage)); // 전체 페이지  
    int totalGrp = (int)(Math.ceil((double)totalPage/pagePerBlock));// 전체 그룹 
    int nowGrp = (int)(Math.ceil((double)nowPage/pagePerBlock));    // 현재 그룹 
    int startPage = ((nowGrp - 1) * pagePerBlock) + 1; // 특정 그룹의 페이지 목록 시작  
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동 
    if (nowGrp < totalGrp){ 
      str.append("<span class='span_box_1'><A href='./list4.do?col="+col+"&word="+word+"&nowPage="+_nowPage+"'>다음</A></span>"); 
    } 
    str.append("</DIV>"); 
