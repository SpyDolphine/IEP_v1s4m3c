...
public class GurumeCont {
  ...
  public ModelAndView create(GurumeVO gurumeVO, HttpServletRequest request, HttpSession session) {
    // System.out.println("--> crte() POST called.");

    ModelAndView mav = new ModelAndView();
    mav.setViewName("/message"); // 상으니 메시지 사용시 기존 메시지파일 삭제하고 콘트 /gurume 부분 삭제 후
                                 // /message 파일사용
    ...
    if (file2MF.getSize() > 0) {
      ...
      if (Tool.isImage(file2)) {
        file1 = Tool.preview(upDir, file2, 200, 100);
      } else {
        file1 = "";
      }
      // -------------------------------------------------------------------
   ...
   * @return
   */
  @RequestMapping(value = "/gurume/read.do", method = RequestMethod.GET)
   ...
   * @return
   */
  @RequestMapping(value = "/gurume/update.do", method = RequestMethod.GET)
   ...
   * @return
   */
  @RequestMapping(value = "/gurume/update.do", method = RequestMethod.POST)
  public ModelAndView update(GurumeVO gurumeVO, HttpServletRequest request, HttpSession session) {
    ModelAndView mav = new ModelAndView();
    // 게시판에 대한 정보 파악
    ...
    if (gurumeDAO.update(gurumeVO) == 1) {
      ...
      // 수정후 조회로 자동 이동
      mav.setViewName("redirect:/gurume/read.do?gu_no="
          + gurumeVO.getGu_no()/* + "&divisionno=" + gurumeVO.getDivisionno() */); // 확장자
                                                                                   // 명시
    } else {
   ...
   * @return
   */
  @RequestMapping(value = "/gurume/delete.do", method = RequestMethod.POST)
  public ModelAndView delete2(int gu_no) {
    ...
    if (gurumeDAO.delete(gu_no) == 1) {
      ...
      mav.setViewName(
          "redirect:/gurume/list4.do" /* + gurumeVO.getDivisionno() */);// 확장자
                                                                        // 명시
      ...
      links.add("<button type='button' onclick=\"history.back()\">다시시도</button>");
      links.add("<button type='button' onclick=\"location.href='./home.do'\">홈페이지</button>");
      links.add("<button type='button' onclick=\"location.href='./list.do"
  ...
  @RequestMapping(value = "/gurume/list4.do", method = RequestMethod.GET)
  public ModelAndView list4(SearchDTO searchDTO, HttpServletRequest request) {
    ModelAndView mav = new ModelAndView();
    ...
    /*
     * GurumeVO gurumeVO = gurumeDAO.read(gu_no); mav.addObject("gurumeVO",
     * gurumeVO);
     */
    ...
    String paging = new Paging().paging725(totalRecord, searchDTO.getNowPage(), recordPerPage, searchDTO.getCol(),
        searchDTO.getWord());
