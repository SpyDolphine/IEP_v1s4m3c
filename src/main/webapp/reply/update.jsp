...
<FORM name='frm' method='POST' action='./update.do'>
  <input type="hidden" id='cm_no' name='cm_no' value="${replyVO.cm_no }">
  <input type="hidden" id='rno' name="rno" value="${replyVO.rno }"> 
  <input type="hidden" name="rnick" value="nick"> <!-- 임시값 -->
    ...
    ${replyVO.rcontent }</textarea>
    <ul>
    <li class='right'>
    <button type="submit" id='submit' class="btn btn-success btn-xs" style='float:right; margin: 5px;'>
    <i class="fa fa-edit"></i> 수정</button>
    <button type="button" class="btn btn-info btn-xs" style='float:right; margin: 5px;' onclick="location.href='./replylist.do?cm_no=${replyVO.cm_no}'">
