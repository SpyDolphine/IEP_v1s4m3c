...
<div class="container">
 <div class="row" align='center'>
   ...
   <!-- 게시글 상단 버튼 -->
     ...
     <c:choose>
     <c:when test="${ifreeVO.cm_no == minlist }">
     ...
     <c:choose>
     <c:when test="${ifreeVO.cm_no == maxlist }">
    ...
    <div class="write_content" style='width: 90%;'>
      <input type="hidden" name="cm_no" value="${ifreeVO.cm_no}">
      <div style='text-align: left;'>&nbsp;&nbsp;${ifreeVO.cm_title}</div><p style='text-align: right;'>
        <span style='color: #cccccc;'>${ifreeVO.cm_date.substring(0, 16)}</span> |
        <A href="./update.do?cm_no=${ifreeVO.cm_no}">수정</A> |
        <A href="./delete.do?cm_no=${ifreeVO.cm_no }&col=${searchDTO.col}&word=${searchDTO.word}&nowPage=${searchDTO.nowPage}"
            onclick="return confirm('삭제 하시겠습니까?')">삭제</A>
      </p>      
      <hr>
      &nbsp;<i class="fa fa-leaf"> ${ifreeVO.cm_nick}</i>
      <fieldset style='margin: 50px 0 0 0;'>
        <ul>
          ...
          <li>
            ...
            <span>
            <c:if test="${ifreeVO.size2 > 0}">
             <A href='${pageContext.request.contextPath}/download?dir=/ifree/storage&filename=${ifreeVO.file2}'>${ifreeVO.file2}</A> (${ifreeVO.size2}KB)
            </c:if>
            </span>    
            <div id='fild2Panel'>
              <c:set var='file2' value="${fn:toLowerCase(ifreeVO.file2)}" />
              <c:choose>
                <c:when test="${fn:endsWith(file2, '.jpg')}">
                  <IMG id='file2' src='./storage/${ifreeVO.file2}' >
                </c:when>
      ...
      <div class='right'>
        <button type="button" class="btn btn-default btn-xs" onclick="location.href='./reply.do?cm_no=${ifreeVO.cm_no}'">답글</button>
